...
public partial class OpenApiComponentServiceTests : OpenApiDocumentServiceTestBase
{
    ...
    [Fact]
    public async Task GetOpenApiRequestBody_GeneratesSchemaForIEnumerableTypes()
    {
        // Arrange
        var builder = CreateBuilder();
        ...
        await VerifyOpenApiDocument(builder, document =>
        {
            ...
            var streamRequestBody = enumerableTodo.RequestBody;
        });
    }

    [Fact]
    public async Task GetOpenApiResponse_HandlesPolymorphicRequestWithoutDiscriminator()
    {
        // Arrange
        var builder = CreateBuilder();
