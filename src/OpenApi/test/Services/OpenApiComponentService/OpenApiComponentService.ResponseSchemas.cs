...
#nullable restore
    ...
    public async Task GetOpenApiResponse_HandlesPolymorphicResponseWithDiscriminator()
    {
        ...
        // Act
        builder.MapGet("/api-2", () => new Shape { Color = "Blue", Sides = 3 });
        builder.MapGet("/api", () => new Shape { Color = "Red", Sides = 3 });
        ...
        await VerifyOpenApiDocument(builder, document =>
        {
            ...
            var anyOf = mediaType.Schema.AnyOf;
            Assert.Collection(anyOf,
                schema =>
                {
